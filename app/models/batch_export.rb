class BatchExport < ActiveRecord::Base
  ...
  def load_primary_cancer_abstractor_abstraction_groups(options={})
    ...
    Abstractor::AbstractorAbstractionGroup.not_deleted.where('histology.abstractor_subject_id = ? AND site.abstractor_subject_id = ? AND batch_export_details.batch_export_id = ?', abstractor_subject_has_cancer_histology.id, abstractor_subject_has_cancer_site.id, self.id).joins("JOIN batch_export_details ON abstractor_abstraction_groups.id = batch_export_details.abstractor_abstraction_group_id JOIN pathology_cases ON abstractor_abstraction_groups.about_id = pathology_cases.id AND abstractor_abstraction_groups.about_type = 'PathologyCase' JOIN abstractor_abstraction_group_members ON abstractor_abstraction_groups.id = abstractor_abstraction_group_members.abstractor_abstraction_group_id JOIN abstractor_abstractions AS histology ON abstractor_abstraction_group_members.abstractor_abstraction_id = histology.id AND histology.deleted_at IS NULL JOIN abstractor_abstraction_group_members site_group_member ON abstractor_abstraction_groups.id = site_group_member.abstractor_abstraction_group_id JOIN abstractor_abstractions AS site ON site_group_member.abstractor_abstraction_id = site.id AND site.deleted_at IS NULL").select('abstractor_abstraction_groups.*, pathology_cases.accession_number, pathology_cases.encounter_date, histology.value AS histology, site.value AS site').order(sort)
  end
